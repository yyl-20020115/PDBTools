...
VOID extdef(BOOL cextdef, USHORT sclass)
{
    ...
    while (reclen > 1) {
        if (cextdef) {
            ...
            name = (sclass == IMAGE_SYM_CLASS_STATIC) ? get_string(FALSE) : get_string (TRUE);
        }
        typ = (USHORT)get_index();
...
VOID fixupp(void)
{
    ...
    while (reclen > 1) {
        ...
        if (TRD_THRED(c) == (USHORT)-1) {
            ...
            if (!FIX_P(c)) {
                offset[i] = use32 ? get_dword_field() : (long)get_word_field();
...
VOID pubdef(USHORT sclass)
{
    ...
    grp = (USHORT)get_index();
    scn = get_index();

    if (!scn) {
        scn = IMAGE_SYM_ABSOLUTE;
        frame = get_word_field();
    } else {
        scn = segindx[scn].scn;
    }

    while (reclen > 1) {
        name = get_string(TRUE);
        value = use32 ? get_dword_field() : (long)get_word_field();
        type = (USHORT) get_index();
...
VOID ledata(void)
{
    ...
    dattype = LEDATA;
    datscn = get_index();
    datoffset = use32 ? get_dword_field() : (ULONG)get_word_field();
...
long expand(long offset)
{
    ...
    repcnt = use32 ? get_dword_field() : (long)get_word_field();
    blkcnt = (long)get_word_field();
...
VOID coment(void)
{
    ...
    flags = get_byte_field();
    class = get_byte_field();
    switch (class) {
        case COM_PRECOMP:
            fPrecomp = TRUE;
            break;
        ...
        case COM_LZEXT:
            while (reclen > 1) {
                weakExt = (USHORT)extdefs[get_index()];
                defaultExt = (USHORT)extdefs[get_index()];
...
VOID segdef(void)
{
    ...
    if (!ACBP_A(acbp)) {
        ...
        frame = get_word_field();
        offset = get_byte_field();
    }
    size = use32 ? get_dword_field() : (long)get_word_field();
    ...
    get_index();                           // Skip overlay LNAME index
...
VOID linnum(void)
{
    ...
    grpindex = (USHORT)get_index();
    segindex = get_index();

    // store lineno data in core until we process entire set

    while (reclen > 1) {
        lineno = get_word_field();
        offset = use32 ? get_dword_field() : (long)get_word_field();
...
VOID nbkpat(void)
{
    ...
    loctyp = get_byte_field();
    name = lname[get_index()];
    while (reclen > 1) {
        offset = value = 0;
        offset = use32 ? get_dword_field() : (ULONG)get_word_field();
        value = use32 ? get_dword_field() : (ULONG)get_word_field();
